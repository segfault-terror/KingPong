...
type MuteUserProps = {
    channelName: string;
    setShowMuteModal: (val: boolean) => void;
    setUsernameToMute: (val: string) => void;
    setShowMuteDialog: (val: boolean) => void;
};
...
export default function MuteUserModal({
    ...
    const { data: muteList, isLoading } = useQuery({
        queryKey: ['channel', channelName, 'mute-list'],
        queryFn: async () => {
            const { data } = await axios.get(
                `/api/chat/channel/${channelName}/mute-list`,
                            ...
                            {results.map((result, idx) => (
                                <button
                                    onClick={() => {
                                        ...
                                        setUsernameToMute(result.username);
