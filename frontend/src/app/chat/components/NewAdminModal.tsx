...
type NewAdminProps = {
    ...
    setShowNewAdminModal: (val: boolean) => void;
    setShowNewAdminDialog: (val: boolean) => void;
};

export default function NewAdminModal(props: NewAdminProps) {
    const { data: channelMembers, isLoading } = useQuery({
        queryKey: ['channel', props.channelName, 'admins'],
        queryFn: async () => {
    ...
    return (
        <>
            <Modal
                onClose={() => props.setShowNewAdminModal(false)}
                childrenClassName="bg-gradient-to-br from-primary to-background p-6 rounded-2xl
                                    border-r border-l border-secondary-500 h-[300px]
                                    lg:w-2/3 w-[90%] max-w-[600px]"
            >
                <div
                    ...
                    {results.length !== 0 && (
                        ...
                        >
                            {results.map((result, idx) => (
                                <button
                                    onClick={() => {
                                        props.setShowNewAdminModal(false);
                                        props.setNewAdminUsername(
                                            result.username,
                                        );
                                        props.setShowNewAdminDialog(true);
                                    }}
                                    key={idx}
