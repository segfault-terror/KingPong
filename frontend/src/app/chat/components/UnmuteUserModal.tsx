...
type UnmuteUserProps = {
    channelName: string;
    setShowUnmuteModal: (val: boolean) => void;
    setUsernameToUnmute: (val: string) => void;
    setShowUnmuteDialog: (val: boolean) => void;
};

export default function UnmuteUserModal({
    setShowUnmuteModal,
    channelName,
    setUsernameToUnmute,
    setShowUnmuteDialog,
}: UnmuteUserProps) {
    const { data: muteList, isLoading } = useQuery({
        queryKey: ['channel', channelName, 'unmute-list'],
        queryFn: async () => {
            const { data } = await axios.get(
                `/api/chat/channel/${channelName}/unmute-list`,
    ...
    useEffect(() => {
        if (!muteList?.members || !muteList?.admins) return;

        setResults(filterMembers(muteList.members, muteList.admins, ''));
    }, [muteList]);
        ...
        <>
            ...
            >
                ...
                >
                            ...
                            {results.map((result, idx) => (
                                <button
                                    onClick={() => {
                                        ...
                                        setUsernameToUnmute(result.username);
                    ...
                    {muteList?.admins.length === 0 &&
                        muteList?.members.length === 0 && (
                            <>
